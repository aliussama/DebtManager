...
public sealed class SchemaInitializer
{
    public async Task InitializeAsync(SqliteConnection connection, CancellationToken ct)
    {
        ...
        CREATE INDEX IF NOT EXISTS ix_events_stream_effective_occurred
            ON events(stream_id, effective_date, occurred_at);

        CREATE INDEX IF NOT EXISTS idx_events_occurred
            ON events(occurred_at);

        CREATE INDEX IF NOT EXISTS ix_events_effective_date
            ON events(effective_date);
        ...
        CREATE INDEX IF NOT EXISTS ix_snapshots_name_date_created
            ON projection_snapshots(projection_name, as_of_date, created_at DESC);
        """;

        await ExecuteAsync(connection, sqlSnapshots, ct);
    }

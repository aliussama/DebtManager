...
public interface IRulePackRepository
{
    ...
    Task<IReadOnlyList<InstalledRulePackInfo>> GetAllAsync(CancellationToken ct);
}

/// <summary>
/// Information about an installed rule pack with version details.
/// </summary>
public sealed record InstalledRulePackInfo(
    string RulePackId,
    string Name,
    string? Description,
    string VersionLabel,
...
""";
    ...
    public async Task<IReadOnlyList<InstalledRulePackInfo>> GetAllAsync(CancellationToken ct)
    {
        await using var conn = _factory.Create();
        await conn.OpenAsync(ct);
        await new Persistence.SchemaInitializer().InitializeAsync(conn, ct);

        var results = new List<InstalledRulePackInfo>();
...
SELECT 
    ...
    rpv.rules_json
FROM rule_packs rp
INNER JOIN rule_pack_versions rpv ON rp.rule_pack_id = rpv.rule_pack_id
ORDER BY rp.name, rpv.effective_from DESC;
""";

        await using var r = await cmd.ExecuteReaderAsync(ct);
        while (await r.ReadAsync(ct))
        {
            var packId = r["rule_pack_id"].ToString()!;
            var name = r["name"].ToString()!;
            var description = r["description"]?.ToString();
            var versionLabel = r["version_label"].ToString()!;
            var effectiveFrom = DateOnly.Parse(r["effective_from"].ToString()!);
            
            var toStr = r["effective_to"]?.ToString();
            DateOnly? effectiveTo = string.IsNullOrWhiteSpace(toStr) ? null : DateOnly.Parse(toStr);
            
            var status = r["status"].ToString()!;
            var rulesJson = r["rules_json"].ToString()!;

            // Count rules in JSON
            var rulesCount = CountRulesInJson(rulesJson);

            results.Add(new InstalledRulePackInfo(
                packId, name, description, versionLabel,
                effectiveFrom, effectiveTo, status, rulesCount
            ));
        }
    ...
    private static int CountRulesInJson(string rulesJson)
    {
        try
        {
            using var doc = System.Text.Json.JsonDocument.Parse(rulesJson);
            if (doc.RootElement.TryGetProperty("rules", out var rulesEl) && 
                rulesEl.ValueKind == System.Text.Json.JsonValueKind.Array)
            {
                return rulesEl.GetArrayLength();
            }
        }
        catch
        {
